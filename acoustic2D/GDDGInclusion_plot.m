...
% OPOP.dg.cubature = CubatureVolumeMesh2D(intC);
...
%}
for k = [5,10,15,30]
  eval(sprintf('load /Users/jekozdon/scratch/gddg/data_n%d_r%d_t%d',...
               2*p+1, sr, k))
  figure(k)
  gd_contourf(OP.B, pr, false, 100);
  cmap
  ...
  eval(sprintf('print -dpng frame_n%d_r%d_t%d.png', 2*p+1, sr, k))
end
